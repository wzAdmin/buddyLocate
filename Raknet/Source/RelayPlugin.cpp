...
void RelayPlugin::JoinGroup(RP_Group* room, StrAndGuidAndRoom **strAndGuidSender)
{
...
void RelayPlugin::JoinGroup(RakNetGUID userGuid, RakString roomName)
{
	...
	if (strAndGuidSender)
	{
		...
		for (unsigned int i=0; i < chatRooms.Size(); i++)
		{
			...
			{
				// Join existing room
				JoinGroup(chatRooms[i],strAndGuidSender);
				return;
			}
		}
		...
		JoinGroup(room,strAndGuidSender);
	}
}
...
void RelayPlugin::OnJoinGroupRequestFromClient(Packet *packet)
{
	...
	JoinGroup(packet->guid, groupName);
}
